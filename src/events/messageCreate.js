...
module.exports = {
  ...
  execute: async (message) => {
      ...
      if (command) {
        if (command.cooldown) {
          if (cooldown.has(`${command.name}${userID}`)) {
            message
              .reply({
                content: `Command on Cooldown \`${ms(
                  ...
                  .replace("ms", `ms`)}\` coba lagi nanti OwO`,
    ...
    function storeMessage() {
      const messagedb = client.db_userMessage;
      if (message.content === "" || message.guildId !== config.guildID) return;
      const dataToStore = {
        author: {
      ...
      messagedb.set(message.id, dataToStore);
    }

    function autoDeleteAmariBotMessage() {
      if (message.guildId !== config.guildID || userID != "339254240012664832")
        return;
      const ignoreChannel = [
    ...
    async function antiSpam() {
      ...
      if (
        message.member.roles.cache.find((r) => ignoreRole.indexOf(r.id) !== -1)
      )
        return;
      if (
        message.member
      ...
      const spamDetected = async (user, channel, msg = "") => {
        ...
        message.channel.send(
          `<@${user}>, muted 10s! no spam ngab <:nindy_ho:977817574500859944>`
        );
      ...
      if (message.attachments.size !== 0) {
        ...
        if (message.attachments.size >= 3) {
          ...
          if (
            !(
              new Set(imgSize).size !== 1 ||
              new Set(imgWidth).size !== 1 ||
              new Set(imgHeight).size !== 1
            )
          ) {
          ...
          let imgSize = Object.values(
            Object.fromEntries(message.attachments)
          )[0].size;
      ...
      if (message.stickers.size !== 0) {
        let stickerID = Object.values(Object.fromEntries(message.stickers))[0]
          .id;
      ...
      if (userset.has(userID + message.content)) {
        ...
        const whitelistedMessage = ["m!skip", "m!s"];
    ...
    async function automodWarn() {
      ...
      await member.send(
        `Our AutoMod detected you send a message that contains a forbidden word: \`${matchMessage}\`, with a rule: \`${rule}\`. If you keep doing this, you will be kicked from NTC Department.\n If you want to share a discord link, please separated it or contact admin (❁´◡\`❁), Thank you!`
      );
    }
    ...
    async function autoDetectBooster() {
      ...
      await _serverBoost.run(
        client,
        message,
        [userID],
        null,
        config.staffCommand_LogChannelID
      );
    ...
    async function trakteerWebhookTest() {
      ...
      const webhookClient = new WebhookClient({
        url: process.env.TEST_WEBHOOK_DONATUR,
      });
    ...
    async function yearOfServiceRole() {
      ...
      if (blacklistedChannel.includes(message.channelId) || userYear[0] === 0)
        return;
      const userRoleCache = message.member.roles.cache;
      ...
      if (userYear[0] === 1) {
        if (await userRoleCache.find((r) => r.id === config.oneYearServiceRole))
          return;
        ...
        if (await userRoleCache.find((r) => r.id === config.twoYearServiceRole))
          return;
        try {
        ...
        if (
          await userRoleCache.find((r) => r.id === config.threeYearServiceRole)
        )
          return;
        try {
          await message.member.roles.remove(config.oneYearServiceRole);
