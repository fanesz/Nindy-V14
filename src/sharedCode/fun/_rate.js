module.exports = {
  ...
  run: async (client, message, args, interaction) => {
    ...
    const randomBintang =
      listBintang[Math.floor(Math.random() * listBintang.length)];
    ...
    if (interaction) {
      ...
      setTimeout(async () => {
        await interaction.editReply(
          `<:nindy_syap:977817690121076746> Nindy rate kamu ${randomBintang}`
        );
      }, 2600);
    } else {
      // prefix
      message
        ...
        .then((m) => {
          ...
          setTimeout(() => {
            m.edit({
              content: `:drum: :drum: :drum:`,
              allowedMentions: { repliedUser: false },
            });
          }, 1700);
          setTimeout(() => {
            m.edit({
              content: `<:nindy_syap:977817690121076746> Nindy rate kamu  ${randomBintang}`,
              allowedMentions: { repliedUser: false },
            });
          }, 2600);
        });
