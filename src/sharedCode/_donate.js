...
module.exports = {
  ...
  run: async (client, message, args, interaction, donateEmbed) => {
    ...
    if (!donateEmbed) {
      ...
      donateEmbedMessage = await client.channels.cache.get(config.donatur_LogChannelID).messages.fetch(embedID);
    } else {
      ...
      try {
        await message.channel.guild.members
          .fetch({ cache: false }).then(members => members
            .find(member => member.user.username === ordernama)).then((result) => {
    ...
    ordermessage = `"${(ordermessage.endsWith(' ')) ? ordermessage.slice(0, -1) : ordermessage}"`;
