...
module.exports = {
  ...
  run: async (client, message, args, interaction) => {
    ...
    if (isNaN(member)) {
      replyMessage(commandType, 'Field `member` must be in number, sir!', null, false, false);
      return;
    }
    ...
    if (commandType.member.voice.channel === null) {
      replyMessage(commandType, 'You must be in a voice channel, sir!', null, false, false);
      return;
    }
    ...
    if (usersInChannel.length < member + (exclude ? exclude.length : 0)) {
      replyMessage(commandType, `Kocak, mana bisa bikin tim ${member} orang tapi di voice cuma ${usersInChannel.length} orang${exclude ? ` sama ${exclude.length} di exclude` : ''}（︶^︶）`, null, false, false);
      return;
    }
    ...
    const team = Array.from(randomSet).map((user, index) => `${index + 1}. <@${user}>`).join('\n')
