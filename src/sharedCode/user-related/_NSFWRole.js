...
module.exports = {
  ...
  run: async (client, message, args, interaction) => {
    ...
    if (
      commandType.member.roles.cache.find((r) => r.id === NSFWRoleID) ===
      undefined
    ) {
      await commandType.member.roles.add(NSFWRoleID);
      messageContent =
        "<:nindy_yes:977817511821213757> done, role NSFW **added**!";
    } else {
      await commandType.member.roles.remove(NSFWRoleID);
      messageContent =
        "<:nindy_yes:977817511821213757> done, role NSFW **removed**!";
    }
