  ...
  run: async (client, message, args, interaction) => {
    ...
    async function sendAll(link) {
      return await axios.get(link).then((res) => {
        ...
        getEpisodes();
        let splitEpisodes = $("#contentWrapper")
          .find("div.spaceit_pad")
          .eq(getEpisodesmod)
          .text()
          .split("\n");
        ...
        getStatus();
        let splitStatus = $("#contentWrapper")
          .find("div.spaceit_pad")
          .eq(getStatusmod)
          .text()
          .split("\n");
        ...
        getGenres();
        let genrelen = $("#contentWrapper")
          .find("div.spaceit_pad")
          .eq(getGenresmod)
          .children("a").length;
        for (i = 0; i <= genrelen - 1; i++) {
          let genre = $("#contentWrapper")
            .find("div.spaceit_pad")
            .eq(getGenresmod)
            .children("a")
            .eq(i)
        ...
        const studio = $("#contentWrapper")
          .find("span.information.studio.author")
          .text()
          .replace("             ", " ");
        ...
        function ifSynopsis() {
          ifSynopsismod++;
          let synopsischeck = $("#contentWrapper")
            .find("p")
            .eq(ifSynopsismod)
            .text();
        ...
        return translation(synopsis).then((result) => {
          ...
          const embed = new EmbedBuilder()
            ...
            .addFields(
              ...
              {
                name: "Synopsis",
                value: synopsisresult + `[click for more](${results[0]})`,
              }
            )
